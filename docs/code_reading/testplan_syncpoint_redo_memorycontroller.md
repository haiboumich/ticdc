# SyncPoint / Redo / Memory Controller 测试方案草案

说明：该测试方案用于支撑“syncpoint + [Memory Controller](#c-terminology) 性能劣化”与“redo 可用性/性能未知”的调研；方案内容遵循 `docs/code_reading/writingstandard.md`。
...
summary：
- 目标：验证 syncpoint + [Memory Controller](#c-terminology) 是否导致 lag 持续上升
 - 同步评估 redo 启用后的性能与稳定性
- 范围与非目标
 - 覆盖新架构链路在 syncpoint/redo 打开时的性能与稳定性。
 - 不覆盖具体实现细节，代码路径详见 `docs/code_reading/memorycontroller.md` / `docs/code_reading/syncpoint.md` / `docs/code_reading/redo.md`。
- 设计反推依据
 - 测试矩阵与观测指标来自上述代码阅读结论（见 B/C 节）。
- 关键角色/组件
 - [EventCollector](#c-terminology) / [DynamicStream](#c-terminology) / [EventService](#c-terminology) / [Dispatcher](#c-terminology) / [Sink](#c-terminology)。
- 关键状态/策略
 - 以测试矩阵（B 节）驱动开关与配额组合，以观测指标（C 节）判定效果。
- 可靠性与降级
 - 重启/重置场景在 D.4 验证恢复语义与可用性。
- 关键假设/前置条件
 - 新架构启用且 changefeed 以配置文件开启 syncpoint/redo（见 H 节）。
- 可观测性
 - 以 DynamicStream 内存、scan skip、redo 进度等指标为主（见 C 节）。

范围：
- 新架构（[EventCollector](#c-terminology) + [DynamicStream](#c-terminology) + [EventService](#c-terminology)）链路
- 下游为 MySQL/TiDB（syncpoint 生效前提）
- redo 启用/关闭对比

涉及组件：[Dispatcher](#c-terminology)与 [Sink](#c-terminology)。
...
- 功能开关：
 ...
 - redo：{off, on}
- 资源配额：
 - [MemoryQuota](#c-terminology)：{小(默认), 中, 大}
 - [ConsistentConfig](#c-terminology).MemoryUsage.MemoryQuotaPercentage：{25, 50(默认), 75}
- 负载模式：
 - DML-only（小事务 / 大事务）
 - DML + DDL 混合（含多表 DDL）
...
summary：
- 优先观测：[DynamicStream](#c-terminology)内存、[EventService](#c-terminology) scan 跳过、redo 进度
- 用于定位 [Memory Controller](#c-terminology) 触发与 lag 变化的因果链路

指标与代码定位：
- DynamicStream memory usage：
 - `metrics.DynamicStreamMemoryUsage` 由 [EventCollector](#c-terminology)维护
...
- redo resolvedTs 进度：
 - [RedoMeta](#c-terminology)驱动的 RedoResolvedTsProgressMessage 上报
 - 代码指针：`downstreamadapter/dispatchermanager/dispatcher_manager_redo.go:285-311`

- 主链路 [redoGlobalTs](#c-terminology)缓存触发：
...
summary：
- 当前调研阻塞条件需先解决
- [Memory Controller](#c-terminology) + syncpoint 组合的性能问题未解决前，redo 性能测试结果可能不稳定

前置条件：
- 建议先收敛 syncpoint + [Memory Controller](#c-terminology) 的性能劣化
- redo 性能测试需在稳定基线后进行
