...
说明：本文仅聚焦关联点与交互路径；[Memory Controller](#c-terminology) 的内部算法与释放链路详见 `docs/code_reading/memorycontroller.md`。
...
summary：
- 关键交互路径：[EventCollector](#c-terminology) -> [DynamicStream](#c-terminology) -> [CongestionControl](#c-terminology) -> [EventService](#c-terminology)
- 关键状态/策略
 - [Memory Controller](#c-terminology) 在主/redo DynamicStream 同时启用。
 - available memory 由 EventCollector 下发并限制 EventService 扫描。
- 范围与非目标
 - 覆盖 SyncPoint/Redo 与 [Memory Controller](#c-terminology) 的交互路径与影响。
 - 不覆盖 Memory Controller 内部算法细节（见 `docs/code_reading/memorycontroller.md`）。
- 关键角色/组件
 - EventCollector 汇总 available memory，DynamicStream 提供内存反馈，EventService 依据 available memory 限制 scan。
- 关键假设/前置条件
 - 新架构启用 EventCollector 且动态流开启 memory control（见 A.1）。
- 可观测性
 - available memory 与 scan skip 分支是关键观测点（见 A.4）。
- 可靠性与降级
 - available memory 不足会跳过 scan 并发送 resolvedTs（见 A.4）。
...
```
说明：[DynamicStream](#c-terminology) 启用 [Memory Controller](#c-terminology) 后会向 EventCollector 输出反馈指标。

<a id="sec-a2-quota"></a>
### A.2 配额注入：DispatcherManager -> DynamicStream

summary：
- [DispatcherManager](#c-terminology)把 changefeed 侧 quota 注入到 DynamicStream
...
```
说明：[EventService](#c-terminology)以 available memory 为前置条件，限制扫描负载。

<a id="sec-b-syncpoint"></a>
## B. SyncPoint ↔ Memory Controller 的交互路径

summary：
- [SyncPointEvent](#c-terminology) 是阻塞事件
 - 会降低 DynamicStream 的吞吐，从而触发 [Memory Controller](#c-terminology) 限流
...
summary：
- redo 链路与主链路共享 [Memory Controller](#c-terminology) 的“最小值”
 - redo 压力会降低主链路可用内存
...
summary：
- [redoGlobalTs](#c-terminology)落后时主链路事件缓存
...
summary：
- 现状问题（调研输入）
 - [Memory Controller](#c-terminology) + SyncPoint 组合后出现严重性能劣化（lag 持续上升）
...
summary：
- SyncPoint 与 [Memory Controller](#c-terminology) 相关问题已有记录
...
已知 issue（按主题分组）：
- [Memory Controller](#c-terminology) + SyncPoint 组合问题
 - #1166（已关闭，2025-09-22）：SyncPoint 事件遇到 [Memory Controller](#c-terminology) 暂停 dispatcher 导致 changefeed 卡住，内存持续增长。https://github.com/pingcap/ticdc/issues/1166
...
已知 PR（相关功能/修复）：
- [Memory Controller](#c-terminology) / SyncPoint 相关
 - #362（已合并，2024-10-12）：引入 [Memory Controller](#c-terminology)。https://github.com/pingcap/ticdc/pull/362
