...
summary：
- 链路主线：`ReplicaConfig`（见[术语汇总小节](#c-terminology)） -> EventService（见[术语汇总小节](#c-terminology)） -> EventCollector（见[术语汇总小节](#c-terminology)） -> Dispatcher（见[术语汇总小节](#c-terminology)） -> MySQL Sink（见[术语汇总小节](#c-terminology)）
  - SyncPointEvent（见[术语汇总小节](#c-terminology)）是 BlockEvent，会改变调度节奏
...
```
说明：默认值与最小约束定义在 `ReplicaConfig`（见[术语汇总小节](#c-terminology)）。
...
summary：
- SharedInfo（见[术语汇总小节](#c-terminology)）作为 Dispatcher 共享配置容器
  - SyncPointConfig 仅在 EnableSyncPoint=true 时构造并注入

调用链：
- DispatcherManager（见[术语汇总小节](#c-terminology)）创建
...
```
说明：SharedInfo（见[术语汇总小节](#c-terminology)）负责把 SyncPoint 配置分发给所有 Dispatcher。
...
```
说明：EventCollector（见[术语汇总小节](#c-terminology)）向 EventService 发送 DispatcherRequest，SyncPoint 的起始值由此传入。
...
```
说明：SyncPointEvent 在 EventService（见[术语汇总小节](#c-terminology)）侧生成，按 dispatcher 的 message worker 发送。
...
```
说明：DynamicStream（见[术语汇总小节](#c-terminology)）对 SyncPointEvent 使用 NonBatchable 分组，确保单条阻塞处理。
...
```
说明：Dispatcher（见[术语汇总小节](#c-terminology)）对 SyncPointEvent 采用阻塞处理策略，并在 redo 模式跳过。
